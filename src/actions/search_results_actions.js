...
export const retrieve_search_results =
	(apiKey, movie, minYear, maxYear, typeSelect, pageCount) =>
	async (dispatch) => {
		let searchTerms = {
			apiKey,
			movie,
		...
		try {
			...
			console.log(spaceRemovedMovie);

			let response = await fetch(
				`https://www.omdbapi.com/?apikey=${apiKey}&s=${spaceRemovedMovie}&y=${minYear}-${maxYear}&type=${typeSelect}`
			);
...
export const infinite_scroll = (searchTerms, pageCount) => async (dispatch) => {
	...
	try {
		...
		let response = await fetch(
			`https://www.omdbapi.com/?apikey=${apiKey}&s=${spaceRemovedMovie}&y=${minYear}-${maxYear}&type=${typeSelect}&page=${pageCount}`
		);
		let result = await response.json();
